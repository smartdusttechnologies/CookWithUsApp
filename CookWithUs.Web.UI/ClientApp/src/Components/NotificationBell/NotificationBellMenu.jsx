...
export default function NotificationBellMenu() {
  ...
  const NotificationCount = () => {
    const notificationCount =
      notification.length - readStatus.filter((el) => el == true).length;
  ...
  return (
    <div>
      <Tooltip title="Notifications">
        ...
        >
          <Badge badgeContent={NotificationCount()} color="error">
            <NotificationsIcon sx={{ width: 25, height: 25, color: "white" }} />
          </Badge>
        </IconButton>
      </Tooltip>
      <Menu
      ...
      >
        <MenuItem
          sx={{ fontSize: "22px", fontWeight: "500", display: "block" }}
          onClick={handleClose}
        ...
        {notification.length == 0 ? (
          <MenuItem sx={{ display: "block" }}>No Notification</MenuItem>
        ) : (
          <span></span>
        )}
        {[...notification].reverse().map((el, i) => (
          <MenuItem
            ...
            key={i}
            style={{
              fontWeight: !readStatus[notification.length - 1 - i]
                ? "bold"
                : 500,
              backgroundColor: !readStatus[notification.length - 1 - i]
                ? "rgb(245, 235, 235)"
                : "white",
